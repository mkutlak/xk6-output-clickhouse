...
# backfill_check.sh
#
# Backfills the check_name column in k6.samples from extra_tags['check']
# for rows where check_name is empty but the native k6 'check' tag was
# stored in extra_tags instead (due to the pre-fix bug).
#
# ClickHouse mutations are asynchronous. This script fires the mutation and
# polls system.mutations until it completes.
#
# Usage:
#   ./scripts/backfill_check.sh [options]
#
# Options:
...
#   --dry-run    Show affected row count and SQL without executing (preview only; omit to execute)
...
echo "==> Counting rows with empty check_name but 'check' in extra_tags..."
AFFECTED=$(ch_query "
    SELECT count()
    FROM ${FULL_TABLE}
...
# The mutation SQL:
#   1. Set check_name from extra_tags['check']
#   2. Remove the 'check' key from extra_tags to avoid duplication
MUTATION_SQL="
...
if [[ "$REMAINING" -gt 0 ]]; then
    echo "    WARNING: ${REMAINING} rows still have empty check_name. Check system.mutations for errors." >&2
fi
