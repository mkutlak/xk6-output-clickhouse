...
- **`isRetryableError` has a narrow scope** — only `io.EOF`/`io.ErrUnexpectedEOF`, `driver.ErrBadConn` (stale pooled connection), `net.Error`, and a few message substrings (connection refused/reset, i/o timeout, no such host, network unreachable, broken pipe) are retried; any other pre-commit error (e.g. a bad query) skips retry immediately but is still buffered like any other flush failure
...
- Bumping `.xk6-version` needs no other file changes: `Makefile`, `ci.yaml`'s build-check job and `main.yaml`'s image build all read the file directly. README's quickstart intentionally hardcodes `xk6@latest`, with its own note to pin a tag for reproducible builds.
